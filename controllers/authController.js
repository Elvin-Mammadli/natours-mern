...
exports.protect = catchAsync(async (req, res, next) => {
  // 1) Get token anc check if it exists
  let token;
  if (
  ...
  if (!token) {
    return next(new AppErr('You are not logged in, please log in', 401));
  }
  ...
  (...roles) =>
    (req, res, next) => {
      if (!roles.includes(req.user.role)) {
        return next(new AppErr('You do not have permission', 403));
      }
      next();
    };
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    return next(
      new AppErr('There was an error sending email. Try again later.'),
      500
    );
  }
});
...
exports.updatePassword = catchAsync(async (req, res, next) => {
  ...
  if (
    ...
    return next(new AppErr('Current password is wrong.', 401));
  }
